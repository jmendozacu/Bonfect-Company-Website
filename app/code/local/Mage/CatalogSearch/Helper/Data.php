...
class Mage_CatalogSearch_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getSynonyms($splitInText)
    {
        $synonymsInQuery = array();
        foreach ($splitInText as $QueryText )
        {
            $temp = Mage::getModel('catalogsearch/query')
                ->loadByQuery($QueryText);
            if($temp->getSynonymFor()) {
                $synonymsInQuery[$QueryText] = $temp->getSynonymFor();
            }
        }
        return $synonymsInQuery;
    }
